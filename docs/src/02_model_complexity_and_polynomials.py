...
# ```mermaid
# flowchart TD
#     A["Raw Input Data: x ∈ ℝ, y = f(x) + ε"] --> B["Polynomial Basis Expansion: Φ(x) = [1, x, x², ..., xᵈ]"]
#     B --> C["Vandermonde Design Matrix: X ∈ ℝ^(N × (d+1))"]
#     C --> D1["SVD Conditioning Pipeline"]
#     C --> D2["Bootstrap Estimation Pipeline (B=500)"]
#     D1 --> E1["Thin SVD: X = U Σ Vᵀ"]
#     E1 --> F1["Singular Spectrum Decay: σ₁ ≥ ... ≥ σ_(d+1)"]
#     F1 --> G1["Condition Number: κ(X) = σ₁ / σ_(d+1)"]
#     D2 --> E2["Fit OLS Estimators: ŵ_b = (X_bᵀ X_b)⁻¹ X_bᵀ y_b"]
#     E2 --> F2["Prediction Ensemble: f̂_b(x_test)"]
#     G1 & F2 --> H["Error & Stability Diagnostics"]
#     H --> I1["Bias² & Variance Decomposition (ddof=0)"]
#     H --> I2["Expected Weight Norm E[||ŵ||₂] & Solver Truncation"]
#     H --> I3["Total Empirical Generalization MSE"]
# ```
...
# - **Overfitting & Ill-Conditioning ($d \ge 8$)**: $\text{Bias}^2$ plateaus before rising at extreme degrees due to rank truncation. Variance explodes ($\approx 26.93$ at $d=14$) as $\kappa(X)$ exceeds $10^{11}$. The unconstrained theoretical weight norm explodes, but standard SVD solvers (*gelsd) truncate trailing singular modes below $\text{rcond} \approx \epsilon_{\text{mach}}$, clamping the empirical norm $E[\Vert{}\hat{w}\Vert{}_2]$ while destabilizing out-of-sample predictions.
